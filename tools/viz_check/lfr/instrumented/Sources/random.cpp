...
// Instrumentation: when env var LFR_TRACE_PATH is set, every ran4()
// uniform draw is appended to that file (one double per line, %.17g).
// Lazy-opened on first call; flushed per-write so a crash mid-run still
// leaves a usable prefix.
static FILE* _lfr_trace_fp = (FILE*)1;   // sentinel: not-yet-resolved

static inline void _lfr_trace_init() {
    const char* p = getenv("LFR_TRACE_PATH");
    if (p && *p) {
        _lfr_trace_fp = fopen(p, "w");
    } else {
        _lfr_trace_fp = NULL;
    }
}

static inline void _lfr_trace_log(double v) {
    if (_lfr_trace_fp == (FILE*)1) _lfr_trace_init();
    if (_lfr_trace_fp) {
        fprintf(_lfr_trace_fp, "%.17g\n", v);
        fflush(_lfr_trace_fp);
    }
}
